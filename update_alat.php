...
if(empty($_POST['no_alat']) || empty($_POST['merk_access_point']) || empty($_POST['serie_access_point']) || empty($_POST['tipe_wireless']) || empty($_POST['mac_address']) || empty($_POST['ip_address'])) {
	...
	if(mysqli_query($koneksi, "UPDATE access_point SET merk_access_point='$merk_access_point', serie_access_point='$serie_access_point', tipe_wireless='$tipe_wireless', mac_address='$mac_address', ip_address='$ip_address' WHERE no_alat='$no_alat'")) {
		header('location:alat.php?edit=success');
	} else {
		header('location:alat.php?edit=gagal');
	}
}
?>